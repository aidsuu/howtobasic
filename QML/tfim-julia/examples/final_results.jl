...
function write_summary(path::AbstractString, main_rows, critical_rows, correlation_rows, entropy_rows)
    ...
    open(path, "w") do io
        ...
        println(io)
        println(io, "## Main Benchmark")
        println(io)
        println(io, "- Exact diagonalization is used for `N = 4, 6, 10` as the reference baseline.")
        println(io, "- The exact-vector product-state ansatz is used only for small `N` as a simple mean-field-like baseline.")
        println(io, "- MPS/DMRG is used for `N = 10, 20, 40` so that larger systems can be treated.")
        println(io)
        println(io, "## Where The Product State Fails")
        println(io)
        println(io, "- On the overlap system `N = 10`, the largest product-state energy gap relative to exact diagonalization appears near `h/J = $(round(worst_product_row.h_over_J, digits = 2))`, with a difference of `$(round(worst_product_row.gap, digits = 6))`.")
        println(io, "- This is consistent with the expected limitation of a product state when correlations and entanglement become stronger near the crossover region.")
        println(io)
        println(io, "## Where MPS Matches Exact Diagonalization")
        println(io)
        println(io, "- On the overlap system `N = 10`, the MPS energy reproduces the exact result with a maximum deviation of `$(round(max_mps_exact_gap, digits = 10))` across the main sweep.")
        println(io, "- For `N = 4` and `N = 6`, earlier benchmarks also showed that the MPS result is effectively identical to exact diagonalization within numerical tolerance.")
        println(io)
        println(io, "## Near-Critical Pattern")
        println(io)
        println(io, "- In the dense sweep, the magnitude of `dMx/d(h/J)` for `N = 40` reaches its maximum near `h/J = $(round(max_derivative_row.h_over_J, digits = 2))`, which marks the sharpest crossover.")
        println(io, "- `Mz` decreases and `Mx` increases as the transverse field grows, consistent with the shift from Ising-dominated order toward x-polarization.")
        println(io)
        println(io, "## Correlations")
        println(io)
        println(io, "- The `zz`, `xx`, and connected `zz` correlation profiles are stored for `h/J = 0.6, 1.0, 1.4` and `N = 20, 40`.")
        println(io, "- Near `h/J = 1`, the correlation decay becomes slower than it is far from the crossover, especially for the larger systems.")
        println(io)
        println(io, "## Entanglement Entropy")
        println(io)
        println(io, "- The MPS mid-chain bipartite entropy is included as an additional entanglement indicator.")
        println(io, "- For `N = 40`, the largest mid-chain entropy in the main sweep appears near `h/J = $(round(max_entropy_row.h_over_J, digits = 2))`, with value `$(round(max_entropy_row.entropy_mid, digits = 6))`.")
    end
    return path
end
